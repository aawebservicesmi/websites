...
# ==========================================
# CONFIGURATION
# Set DRY_RUN to False when you want to actually start compiling PDFs!
DRY_RUN = False
...
def main():
    mode_label = "DRY RUN (PREVIEW)" if DRY_RUN else "LIVE PDF GENERATION"
    ...
    for folder_name in BUSINESS_MAPPING.keys():
        flyer_dir = os.path.join(ROOT_DIR, folder_name, "Flyer")
        
        if not os.path.exists(flyer_dir):
            skipped_count += 1
            continue
            
        # Find any file starting with 'newLatexTemplate' and ending with '.tex'
        try:
            target_files = [
                f for f in os.listdir(flyer_dir) 
                if f.startswith("newLatexTemplate") and f.endswith(".tex")
            ]
        except Exception:
            continue
        ...
        for tex_file in target_files:
            ...
            print(f"   -> Found file: {tex_file}")
            
            if not DRY_RUN:
                print(f"   ⏳ Running pdflatex on {tex_file}... please wait...")
                
                # We execute pdflatex inside the specific folder via `cwd` 
                # so that auxiliary files (.aux, .log) and the final PDF land in the Flyer folder
                result = subprocess.run(
                    ["pdflatex", "-interaction=nonstopmode", tex_file],
                    cwd=flyer_dir,
                    stdout=subprocess.DEVNULL,  # Keeps terminal output clean
                ...
                if result.returncode == 0:
                    ...
                    print(f"   ❌ ERROR: pdflatex compilation failed for {tex_file}")
            else:
                print(f"   -> Status: WILL COMPILE")
                
            compiled_count += 1
            print("-" * 50)

    print(f"\nProcess complete.")
    print(f"Total templates compiled/targeted: {compiled_count}")
    print(f"Folders skipped (no file matching criteria): {skipped_count}")
    
    if DRY_RUN and compiled_count > 0:
        print("\n*** View looked good? Set 'DRY_RUN = False' to start compiling live PDFs! ***")
