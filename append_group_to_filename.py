...
# Choose "copy" to create a new numbered file next to the original,
# or "rename" to completely rename the original file.
ACTION = "copy" 
...
def main():
    mode_label = "DRY RUN (PREVIEW)" if DRY_RUN else f"LIVE WRITE ({ACTION.upper()})"
    print(f"--- RUNNING IN {mode_label} MODE ---")
    
    processed_count = 0
    skipped_count = 0

    # Pattern captures the exact 3 digits via group(1) inside parenthesis
    ...
    for folder_name in BUSINESS_MAPPING.keys():
        ...
        try:
            ...
            # Find the group number inside the file string
            match = re.search(pattern, content)
            
            if match:
                group_num_str = match.group(1) # e.g. "013"
                new_filename = f"newLatexTemplate{group_num_str}.tex"
                new_file_path = os.path.join(ROOT_DIR, folder_name, "Flyer", new_filename)
                
                print(f"📁 Folder: {folder_name}")
                print(f"   -> Found Group: {group_num_str}")
                print(f"   -> Target File: {new_filename}")
                
                if not DRY_RUN:
                    if ACTION == "copy":
                        shutil.copy2(latex_path, new_file_path)
                        print("   -> Status: COPIED SUCCESS")
                    elif ACTION == "rename":
                        os.rename(latex_path, new_file_path)
                        print("   -> Status: RENAME SUCCESS")
                else:
                    print(f"   -> Status: WILL {ACTION.upper()}")
                    
                processed_count += 1
            else:
                print(f"❌ Folder: {folder_name} -> Missing 3-digit tracking tag sequence inside template.")
                skipped_count += 1
    ...
    print(f"\nProcess complete.")
    print(f"Files handled: {processed_count}")
    print(f"Files skipped: {skipped_count}")
    
    if DRY_RUN and processed_count > 0:
        print(f"\n*** Everything looks stable. Flip 'DRY_RUN = True' to 'DRY_RUN = False' to execute the {ACTION}s! ***")
