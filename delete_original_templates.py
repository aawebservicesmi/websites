...
def main():
    mode_label = "DRY RUN (PREVIEW)" if DRY_RUN else "LIVE DELETE"
    ...
    for folder_name in BUSINESS_MAPPING.keys():
        target_file_path = os.path.join(ROOT_DIR, folder_name, "Flyer", "newLatexTemplate.tex")
        
        if os.path.exists(target_file_path):
            print(f"📁 Folder: {folder_name}")
            print(f"   -> Target: Flyer/newLatexTemplate.tex")
            
            if not DRY_RUN:
                try:
                    os.remove(target_file_path)
                    print("   -> Status: DELETED SUCCESS")
                except Exception as e:
                    print(f"   -> Status: FAILED due to error: {e}")
            else:
                print("   -> Status: WILL DELETE")
                
            deleted_count += 1
            print("-" * 50)
        else:
            # File is missing (already deleted or never existed)
            skipped_count += 1

    print(f"\nProcess complete.")
    print(f"Files deleted/targeted: {deleted_count}")
    print(f"Files missing or already gone: {skipped_count}")
    
    if DRY_RUN and deleted_count > 0:
        print("\n*** If the targets look correct, switch 'DRY_RUN = True' to 'DRY_RUN = False' to execute. ***")
